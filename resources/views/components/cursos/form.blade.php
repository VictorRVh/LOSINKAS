...
<form
    x-data="{
        ...
        validate() {
            ...
            const nombre = this.$refs.nombre_curso.value.trim()
            const descripcion = this.$refs.descripcion.value.trim()

            if (!nombre) {
                this.errors.nombre_curso = 'El nombre del curso es obligatorio.'
            } else if (nombre.length < 3) {
                this.errors.nombre_curso = 'El nombre del curso debe tener al menos 3 caracteres.'
            } else if (nombre.length > 80) {
                this.errors.nombre_curso = 'El nombre del curso no debe superar los 80 caracteres.'
            }
    ...
    <input type="hidden" name="grado_area_id" value="{{ old('grado_area_id', $curso?->grado_area_id ?? $gradoArea?->id) }}">
    ...
    <div>
        ...
        <input
            x-ref="nombre_curso"
            x-on:input="delete errors.nombre_curso"
            name="nombre_curso"
        ...
        <p x-show="errors.nombre_curso" x-text="errors.nombre_curso" x-cloak class="mt-2 text-sm text-red-600"></p>
    ...
    <div>
        ...
        <textarea
            ...
            name="descripcion"
            rows="3"
            class="w-full rounded-none border-2 border-[#0A1718] bg-[#F4F7F7] px-4 py-3 outline-none">{{ old('descripcion', $curso?->descripcion) }}</textarea>
